...
export const addEvent = createAsyncThunk('appCalendar/addEvent', async (event, { dispatch }) => {
  ...
  await dispatch(fetchEvents(['John Doe', 'Adam Smith', 'Alex Cooper']))
...
export const appCalendarSlice = createSlice({
  ...
  initialState: {
    events: [],
    selectedEvent: null,
    selectedCalendars: ['John Doe', 'Adam Smith', 'Alex Cooper']
  },
  reducers: {
    ...
    handleAllCalendars: (state, action) => {
      ...
      if (value === true) {
        state.selectedCalendars = ['John Doe', 'Adam Smith', 'Alex Cooper']
      } else {
